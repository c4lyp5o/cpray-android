...
// define get-zone to listen from front process

back.on('get-times', async function(zone, err){
    if(err) throw err;
    const times = await cpray.getTimesbyWeek(zone);
    const realZone = zoneReplace[zone];
    back.send('give-times', times, realZone);
});